...
async def list_all_courses(refrescar: bool = False, _: dict = Depends(_require_admin_or_academico)):
    """Cursos para la interfaz.

    Por defecto sirve desde la BD sincronizada (instantáneo, sin golpear Canvas).
    Con ?refrescar=1 consulta Canvas, devuelve eso y actualiza la BD.
    """
    import db as _db
    import sync_service
    ...
    async def _desde_bd():
        rows = await _db.fetch(
            """SELECT canvas_course_id, nombre, sis_course_id, semestre, subcuenta,
        ...
        return [{
            ...
            "account_name": r["subcuenta"] or "",
            "workflow_state": r["estado"] or "",
            "total_students": r["total_alumnos"],
            "_fuente": "bd",
            "_ultima_sync": r["ultima_sync"].isoformat() if r["ultima_sync"] else None,
        } for r in rows]

    if not refrescar:
        try:
            cursos = await _desde_bd()
            if cursos:
                return cursos
        except Exception as exc:
            logger.warning("No se pudo leer sync_cursos (%s); consultando Canvas", exc)

    # Refresco explícito, o la BD todavía está vacía
    try:
        cursos = await canvas_service.get_all_courses()
    except Exception as exc:
        logger.warning("Canvas all-courses falló (%s); usando BD", exc)
        try:
            cursos_bd = await _desde_bd()
            if cursos_bd:
                return cursos_bd
        except Exception:
            pass
            ...
            await _db.executemany(
                """INSERT INTO sync_cursos
                   ...
                   ON CONFLICT (canvas_course_id) DO UPDATE SET
                       nombre=EXCLUDED.nombre, sis_course_id=EXCLUDED.sis_course_id,
